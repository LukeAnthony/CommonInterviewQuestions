...
public class LinkedList {
  ...
  public Node findNode(Comparable nodeData) {
    Node traverser = head;
    while (traverser != null) {
      if (traverser.getData() == nodeData) {
        return traverser;
      }
  ...
  public Comparable getNthNode(int n) {
    if (n <= 0)
      ...
      if (count != n && iterator.getNextNode() == null)
        return new String("There aren't enough elements in the list to satisfy you needs");
      else
  ...
  public void reverseList() {
    if (head == null);
      ...
      Node next = head.getNextNode().getNextNode();
      while (current != null) {
